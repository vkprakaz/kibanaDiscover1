...
import './orderview_field_search.scss';
...
/**
 * Component is Orderview's side bar to  search of available fields
 * Additionally there's a button displayed that allows the user to show/hide more filter fields
 */
export function OrderviewFieldSearch({ onChange, value, types, useNewFieldsApi }: Props) {
  const searchPlaceholder = i18n.translate('orderview.fieldChooser.searchPlaceHolder', {
    defaultMessage: 'Search field names',
  });
  const aggregatableLabel = i18n.translate('orderview.fieldChooser.filter.aggregatableLabel', {
    defaultMessage: 'Aggregatable',
  });
  const searchableLabel = i18n.translate('orderview.fieldChooser.filter.searchableLabel', {
    defaultMessage: 'Searchable',
  });
  const typeLabel = i18n.translate('orderview.fieldChooser.filter.typeLabel', {
    defaultMessage: 'Type',
  });
  ...
  const filterBtnAriaLabel = isPopoverOpen
    ? i18n.translate('orderview.fieldChooser.toggleFieldFilterButtonHideAriaLabel', {
        defaultMessage: 'Hide field filter settings',
      })
    : i18n.translate('orderview.fieldChooser.toggleFieldFilterButtonShowAriaLabel', {
  ...
  const buttonContent = (
    ...
    >
      <FormattedMessage
        id="orderview.fieldChooser.fieldFilterButtonLabel"
  ...
  const select = (
    ...
    return (
      <EuiSelect
        ...
        aria-label={i18n.translate('orderview.fieldChooser.filter.fieldSelectorLabel', {
  ...
  const toggleButtons = (id: string) => {
    return [
      {
        id: `${id}-any`,
        label: i18n.translate('orderview.fieldChooser.filter.toggleButton.any', {
          defaultMessage: 'any',
        }),
      },
      {
        id: `${id}-true`,
        label: i18n.translate('orderview.fieldChooser.filter.toggleButton.yes', {
          defaultMessage: 'yes',
        }),
      },
      {
        id: `${id}-false`,
        label: i18n.translate('orderview.fieldChooser.filter.toggleButton.no', {
          defaultMessage: 'no',
        }),
      },
    ];
  ...
  const footer = () => {
    return (
      <EuiPopoverFooter paddingSize="s">
        <EuiSwitch
          label={i18n.translate('orderview.fieldChooser.filter.hideMissingFieldsLabel', {
        ...
        <EuiFilterGroup className="dscFieldSearch__filterWrapper">
          ...
          >
            <EuiPopoverTitle paddingSize="s">
              {i18n.translate('orderview.fieldChooser.filter.filterByTypeLabel', {
                defaultMessage: 'Filter by type',
              })}
            </EuiPopoverTitle>
            {selectionPanel}
