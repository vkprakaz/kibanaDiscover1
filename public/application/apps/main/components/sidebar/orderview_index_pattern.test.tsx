...
import { OrderviewIndexPattern, OrderviewIndexPatternProps } from './orderview_index_pattern';
...
describe('OrderviewIndexPattern', () => {
  test('Invalid props dont cause an exception', () => {
    const props = ({
      indexPatternList: null,
      selectedIndexPattern: null,
      onChangeIndexPattern: jest.fn(),
    } as unknown) as OrderviewIndexPatternProps;

    expect(shallow(<OrderviewIndexPattern {...props} />)).toMatchSnapshot(`""`);
  });
  test('should list all index patterns', () => {
    const instance = shallow(<OrderviewIndexPattern {...defaultProps} />);
