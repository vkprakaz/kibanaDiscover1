...
import { getState } from '../apps/main/services/orderview_state';
import indexTemplateLegacy from './orderview_legacy.html';
import {
  getAngularModule,
  getServices,
  getUrlTracker,
...
app.config(($routeProvider) => {
  const defaults = {
    ...
    requireUICapability: 'orderview.show',
    ...
    badge: () => {
      ...
      return {
        text: i18n.translate('orderview.badge.readOnly.text', {
          defaultMessage: 'Read only',
        }),
        tooltip: i18n.translate('orderview.badge.readOnly.tooltip', {
  ...
  $routeProvider.when('/view/:id?', orderviewRoute);
  $routeProvider.when('/', orderviewRoute);
});

app.directive('orderviewApp', function () {
  return {
    restrict: 'E',
    controllerAs: 'orderviewApp',
    controller: orderviewController,
  };
});

function orderviewController($route, $scope) {
